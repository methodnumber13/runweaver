...
func (c cli) initCmd(args []string) error {
	...
	provider := fs.String("provider", "openai-compatible", "OpenCode provider id for model preflight")
	...
	if classifyOptions.ProviderID == "" || classifyOptions.ProviderID == "openai-compatible" {

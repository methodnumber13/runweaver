...
func TestCLIDoctorModelCommandInfersProviderFromModelPrefix(t *testing.T) {
	root := t.TempDir()
	isolateOpenCodeEnv(t)
	t.Setenv("RUNWEAVER_MODEL_API_KEY", "test-key")
	writeCLIFile(t, root, "opencode.json", `{
  ...
  "provider": {
	...
	code := runCLI([]string{"doctor", "model", "--repo", root}, &stdout, &stderr, false)

	if code != 0 {
		t.Fatalf("doctor model exit code = %d stderr=%q stdout=%q", code, stderr.String(), stdout.String())
	}
	if !strings.Contains(stdout.String(), `"providerId": "company-llm"`) || !strings.Contains(stdout.String(), `"ready": true`) {
		t.Fatalf("stdout = %q, want inferred provider and ready true", stdout.String())
	}
}

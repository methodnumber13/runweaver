...
func TestExecuteWorkflowDryRunWritesPromptAndCommand(t *testing.T) {
  ...
  "phases": [
	...
	for _, want := range []string{"RunWeaver Swarm Execution", "checkpoint.json", "repo-specific participants", "Delegate through OpenCode", "context-discipline", "maxParticipants", "participantRationale", "verificationResults", "runweaver workflow verify"} {
...
func TestExecuteWorkflowCodexDryRunBuildsCodexCommandAndPrompt(t *testing.T) {
  ...
  "phases": [{"id": "plan", "name": "Plan", "scope": "repo", "mode": "parallel", "writeMode": "read", "agents": ["repo-surface-indexer"], "prompt": "plan"}]
	...
	for _, want := range []string{"running through Codex", ".codex/runweaver/profile.json", "Use Codex subagents"} {
		if !strings.Contains(string(prompt), want) {
			t.Fatalf("prompt missing %q:\n%s", want, string(prompt))
		}
	}

...
func TestAdapterMetadata(t *testing.T) {
	adapter := Adapter{}
	if adapter.ID() != catalog.Codex {
		t.Fatalf("ID() = %q, want %q", adapter.ID(), catalog.Codex)
	}
	if adapter.Provider().Binary != "codex" {
		t.Fatalf("provider binary = %q, want codex", adapter.Provider().Binary)
	}
	if adapter.ProfilePath() != ".codex/runweaver/profile.json" {
